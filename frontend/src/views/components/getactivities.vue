...
export default {
  ...
  methods: {
    loadActivities() {
  axiosInstance.get('/dashboard-parents/Activites') // Utilisation de axiosInstance avec l'URL relative
    .then(response => {
      this.products = response.data.map(activity => ({
            id: activity.id,
            name: activity.titre,
            image: activity.image_pub, // Assurez-vous que le backend renvoie bien l'URL de l'image
            price: activity.tarif, // Assurez-vous que le backend renvoie le tarif
            path: `/activity/${activity.id}` // Modifiez selon le besoin
          }));
        })
        .catch(error => {
