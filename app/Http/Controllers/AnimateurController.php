   ...
   // -----------------   PARTIE DE CRUD HORAIRES   ----------------- //
    ...
    public function showHeure($horaire)
    {
        ...
        if( $animateur->horaires()->where('horaire_id',$horaire)->exists())
        {
            return response()->json(
                [
                    'data'=> $animateur->horaires()->where('horaire_id',$horaire)->firstOrFail()
                ]
            );
            ;
        }
     ...
     */
    public function updateHeure($horaire, Request $request) // id horaire, {newHeure_id:string}
    {
        // validation d'inputs : l'horaire forni est une heure
        ...
        if( $animateur->horaires()->where('horaire_id',$horaire)->exists() && 
            ! $animateur->horaires()->where('horaire_id',$request->newHeure_id)->exists())

            $animateur->horaires()->updateExistingPivot($horaire,['horaire_id'=>$request->newHeure_id]);
        else
        return response()->json(
    ...
    public function showEtudiant($activite, $etudiant)
    {
        ...
        foreach ($animateur->getActivites as $act)
        {
            if( $act->id == $activite)
            {
                $enfant = $act->enfants()->findOrFail($etudiant);
                
                return response()->json([
                    // get the student
                    'enfant' => $enfant,
                    'parant' => [
                        'nom' =>($enfant->parentmodel)->user->nom, 
                        'prenom'=>($enfant->parentmodel)->user->prenom,
                        'fonction'=>$enfant->parentmodel->fonction
                        ]
                ]);
            }
        }
